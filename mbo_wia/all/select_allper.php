...
$sql = "SELECT * FROM mbo_wia WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
	echo "<div class=\"table-responsive\">";
    echo "<table class=\"table table-bordered w3-small\"><thead><tr class=\"w3-red\"><th>Serial_No</th><th>File No</th><th>Regiment No</th><th>Rank & Name</th><th>Unit</th><th>Force</th><th>NIC</th><th>Full Name</th><th>Civil Status</th><th>Date of Incident</th><th>Place of Incident</th><th>Mission_Name</th><th>Date of Medical Board Out</th><th>Next of Kin</th></th><th>Home Address</th><th>City</th><th>Province</th><th>Contact(Home)</th><th>Contact(Mob)</th><th>Monthly Sallery</th><th>Compensated/Not</th></tr></thead>";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<tr class=\"w3-hover-green\"><td>$counter</td><td>".$row["File_No"]."</td><td><a href=\"mbo_wia.php?reg=$regi\">".$row["Regiment_No"]."</td><td>".$row["Rank"]." ".$row["Name(Army_Form)"]."</td><td>".$row["Current/Last_Unit"]."</td><td>".$row["Force_"]."</td><td>".$row["NIC_No"]."</td><td>".$row["Full_Name"]."</td><td>".$row["Civil_Status"]."</td><td>".$row["Date_of_Incident"]."</td><td>".$row["Area_that_Incident_Occurred"]."</td><td>".$row["Mission_Name"]."</td><td>".$row["Area_that_Incident_Occurred"]."</td><td>".$row["Next_of_Kin"]."</td><td>".$row["Home_Address"]."</td><td>".$row["City"]."</td><td>".$row["Province"]."</td><td>".$row["Home_Phone"]."</td><td>".$row["Mobile"]."</td><td>".$row["Do_You_Recieve_the_Soldiers_Monthly_Salary"]."</td><td>".$row["Has_the_Soldier_Got_Any_Compensation_From_SL_Army"]."</td></tr>";
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM mbo_wia_spouse WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM mbo_wia_children WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM mbo_wia_parents WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM mbo_wia_brosis WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
