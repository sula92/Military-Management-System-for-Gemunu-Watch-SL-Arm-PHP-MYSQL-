...
echo "<center><h2>ALL OTHER INJURED/SICK DEATH AFTER MEDICAL BOARD OUT SOLDIER'S PARENTS $x  </center></h2>";
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM dambo_other_injured_sick_parents WHERE (`Has_the_Soldier_Got_Any_Compensation_From_SL_Army` LIKE '$yr%') ORDER BY Rank_Seniority_Order";
...
$conn = new mysqli($servername, $username, $password,$dbname);
		...
		$qry="SELECT count(*) as c FROM dambo_other_injured_sick_parents WHERE (`Has_the_Soldier_Got_Any_Compensation_From_SL_Army` LIKE '$yr%')";
...
echo "<h3>TOTAL $x OTHER INJURED/SICK DEATH AFTER MEDICAL BOARD OUT SOLDIER'S PARENTS COUNT IS: $inc</h3>"
