...
$sql = "SELECT * FROM absent WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
	 echo "<div class=\"table-responsive\"><table class=\"table table-bordered w3-small\"><tr class=\"w3-red\"><th>Serial Number</th><th>File No</th><th>Regiment No</th><th>Rank & Name</th><th>Unit</th><th>Force</th><th>NIC</th><th>Full Name</th><th>Civil Status</th><th>Absent Date</th><th>Absent Time</th><th>Next of Kin</th></th><th>Home Address</th><th>City</th><th>Province</th><th>Contact(Home)</th><th>Contact(Mob)</th><th>Monthly Sallery</th><th>Compensated/Not</th></tr></div>";
    // output data of each row
    while($row = $result->fetch_assoc()) {
		
		$regi=$row['Regiment_No'];
		
        echo "<tr><td>".$counter."</td><td>".$row["File_No"]."</td><td><a href=\"absent.php?reg=$regi\">".$row["Regiment_No"]."</a></td><td>".$row["Rank"]." ".$row["Name(Army_Form)"]."</td><td>".$row["Current/Last_Unit"]."</td><td>".$row["Force_"]."</td><td>".$row["NIC_No"]."</td><td>".$row["Full_Name"]."</td><td>".$row["Civil_Status"]."</td><td>".$row["Absent_Date"]."</td><td>".$row["Absent_Time"]."</td><td>".$row["Next_of_Kin"]."</td><td>".$row["Home_Address"]."</td><td>".$row["City"]."</td><td>".$row["Province"]."</td><td>".$row["Home_Phone"]."</td><td>".$row["Mobile"]."</td><td>".$row["Do_You_Recieve_the_Soldiers_Monthly_Salary"]."</td><td>".$row["Has_the_Soldier_Got_Any_Compensation_From_SL_Army"]."</td></tr>";
		$counter=$counter+1;
	}
    echo "</table>";
	echo "</div>";
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM absent_spouse WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM absent_children WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM absent_parents WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
...
mysqli_set_charset($conn,"utf8");
$sql = "SELECT * FROM absent_brosis WHERE (Regiment_No LIKE '$ran%') ORDER BY Rank_Seniority_Order";
$result = $conn->query($sql);
